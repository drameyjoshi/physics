...
\begin{eqnarray}
\curl\vec{E} &=& -\frac{1}{c}\pdt{\vec{B}}{t} \label{c4e1} \\
\dive\vec{B} &=& 0 \label{c4e2}
\end{eqnarray}
These equations can be easily cast in integral form as 
\begin{eqnarray}
\oint\vec{E}\cdot d\vec{l} &=& -\frac{1}{c}\frac{\partial}{\partial t}\int\vec{B}\cdot d\vec{f} \label{c4e3} \\
...
is equal to $-1/c$ times the flux of $\vec{B}$ through the surface bound by the
countour. On the other hand, \eqref{c4e4} tells that flux of $\vec{B}$ through
...
they involve the time derivative of $\vec{B}$ alone and not $\vec{E}$.
...
-\pdt{E_x}{y} + \pdt{E_y}{x} - \frac{1}{c}\pdt{B_z}{t} = 0,
\]
