...
\]
Note that the field makes and angle $2\pi - \theta$ with the positive $x$ axis.
Likewise, the contribution from an element on the left arm is
\begin{eqnarray*}
d\vec{E}_l &=& \frac{dq}{r^2}(\cos\theta\uv{x} + \sin\theta\uv{y}) \\
 &=& \frac{\lambda rd\theta}{r^2}(\cos\theta\uv{x} + \sin\theta\uv{y}) \\
 &=& \frac{\lambda}{r}(\cos\theta\uv{x} + \sin\theta\uv{y})d\theta \\
 &=& \frac{\lambda}{b}(\cos^2\theta\uv{x} + \cos\theta\sin\theta\uv{y})d\theta
\end{eqnarray*}
Therefore,
\[
\vec{E}_l = \int_0^{\pi/2} d\vec{E}_l =
\frac{\lambda}{b}\left(\frac{\pi}{4}\uv{x} + \frac{1}{4}\uv{y}\right).
\]
Similarly, the field due to the right arm is
\[
\vec{E}_r =\frac{\lambda}{b}\left(-\frac{\pi}{4}\uv{x} + \frac{1}{4}\uv{y}\right).
\]
so that $\vec{E}_s + \vec{E}_l + \vec{E}_r = 0$.
\end{enumerate}
\end{document}