...
\]
At what $x$ is $\psi_4(x) = 0$? Points at which $\psi$ vanishes are called nodes.
Thus, there are some points at which the particle just cannot be. This is another
stark difference between the classical and the quantum particles.
\item The normalisation condition is defined by \eqref{c4s1e7}. Why did we change
the limits of integration from $\pm\infty$ to $\pm L/2$?
\end{enumerate}
